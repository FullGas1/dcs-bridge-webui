...
| [`FEAT-ADAPTIVE-LAYOUT-AND-ZOOM`](FEAT-ADAPTIVE-LAYOUT-AND-ZOOM/PRD.md) | ready-for-agent | The editor and the result each reserve a fixed ~30/8-line block regardless of actual content, and only the whole widget can be expanded. Dynamic collapsed-height sizing for both, an independent Expand per area, and a single page-wide zoom (floating control + Ctrl+scroll, 80%–200%). | — |
