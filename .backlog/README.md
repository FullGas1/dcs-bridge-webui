...
| [`FEAT-LUA-FILE-DROP`](FEAT-LUA-FILE-DROP/PRD.md) | merged (PR #13); ticket 05 manual check pending | The only way to get a script into a widget is to paste or type it. Adds drag-and-drop of a `.lua` file from the file manager onto a widget (replace its editor, remember the file's base name) or onto the `+` button (one new pre-filled widget per file). `.lua` only, 512 KB cap, BOM stripped, aggregated self-dismissing message for skipped files. Base name only — no path/handle (ADR 0005). | `feature/lua-file-drop` |
| [`FIX-EDITOR-DROP-HEIGHT`](FIX-EDITOR-DROP-HEIGHT/PRD.md) | merged (PR #14) | A bulk `setValue()` (dropped file, loaded template) made `reportHeight()` measure stale layout → negative collapsed height → CSS drops it → editor grows uncapped and stuck. Clamp the chrome term at zero. Also makes the drop message bar sticky + accented. Found in `FEAT-LUA-FILE-DROP` ticket 05. | `fix/editor-drop-height` |
