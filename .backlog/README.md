...
| [`FEAT-PAGE-SURFACE-CONTRAST`](FEAT-PAGE-SURFACE-CONTRAST/PRD.md) | ready-for-agent | The page background and every card-like surface (widgets, dialogs, dropdown, floating zoom control) share one `--bg` variable, so nothing stands out. Splits it into `--bg-page` (light gray) and `--bg-surface` (unchanged white/dark), symmetric in both light and dark mode. | — |
